...
export function Keys() {
  ...
  const [midiAccessObj, setMidiAccessObj] = React.useState<WebMidi.MIDIAccess | null>(null)
  ...
  const noteOn = React.useCallback((note: number) => {
    if (note === 21) {
      goNext();
    } else if (note === 108) {
      restart();
  ...
  const getMidiMessage = React.useCallback((message: WebMidi.MIDIMessageEvent) => {
  ...
  return (
    <div>
      <Typography sx={{ mb: 2 }}>
        Welcome to this dinky tool! It goes through all 12 keys at random; I use it to practice scales, chord types, etc. without developing muscle memory of moving around the circle of 5ths.
      </Typography>

      <Typography sx={{ mb: 2 }}>
        If your MIDI keyboard is plugged in when accessing this page, the lowest A on the keyboard will go to the next key and the highest C will restart.
      </Typography>
      ...
      <Box sx={{ display: 'flex', gap: 1, my: 2 }}>
        ...
        { hasStarted && <Button onClick={restart} color='warning' variant='outlined'>
          Restart
        </Button> }
      </Box>

      <Typography color='primary' variant='subtitle1' sx={{ fontSize: '2rem' }}>{letterText}</Typography>
