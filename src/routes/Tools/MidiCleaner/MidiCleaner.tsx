...
const CC_TYPES = [
  ...
  {
    code: 81,
    label: 'Hi Pass Filter Frequency or General Purpose Button 2 (on/off) Roland Tone level 2',
  },
  {
    code: 82,
...
export function MidiCleaner() {
  const [checkedState, setCheckedState] = React.useState<{ [key: number]: boolean }>(Object.fromEntries(CC_TYPES.map((item) => ([
    item.code,
    item.checked ?? false
  ]))));
  ...
  const doThing = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      for (let index = 0; index < midiResult.tracks.length; index++) {
        ...
        const trackFiltered = track.filter((midiEvent) =>
      ...
      downloadFile(
        ...
        new Blob([
          Buffer.from(writeMidi(midiResult))
        ])
      )
    }
  ...
  return (
    <div>
      ...
      <Typography sx={{ mb: 2 }}>
        In MuseScore 3, when exporting to MIDI, there seems to be a lot of additional MIDI CC data that also gets exported. I end up clearing out all of this data manually. I decided to build this tool to avoid having to do that.
      </Typography>

      <Typography sx={{ mb: 2 }}>
        Below is a list of all possible MIDI CC data types. <strong>The types that are checked will result in removing MIDI data of that type.</strong> That is, select the types that you want to remove.
      </Typography>
      ...
      <FormGroup>
        <Grid container>
          {
            ITEMS.map((item, idx) => {
              const codeLabel = item.codeMin ? `${item.codeMin}-${item.codeMax}` : item.code;
                ...
                <Grid item key={idx} xs={12} md={6}>
                  <FormControlLabel
                    control={(
                      <Checkbox
                        checked={checked}
                        onChange={() => setCheckedState({ ...checkedState, [item.code]: !checked })}
                      />
                    )}
                    label={<>{codeLabel} &#8212; {item.label}</>}
                  />
                </Grid>
              )
            })
